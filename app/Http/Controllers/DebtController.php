...
class DebtController extends Controller
{
    ...
    {
        $filters = $request->only(['daterange', 'note', 'is_income', 'customer']);
        ...
        return view('Admin.debts.index')->with([
            'expenses' => Expense::query()
                ...
                ->when(array_key_exists('is_income', $filters) && !is_null($filters['is_income']), fn ($q) => $q->where('is_income', $filters['is_income']))
                ->latest()
                ->latest('expense')
                ->paginate(25),
